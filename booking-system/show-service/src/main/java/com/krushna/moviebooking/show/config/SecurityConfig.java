...
/**
 * Hardened Security configuration for show-service.
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Public read-only show queries
                .requestMatchers(HttpMethod.GET, "/api/v1/shows/**").permitAll()

                // Internal service and admin endpoints (show management & seat booking status changes)
                .requestMatchers(HttpMethod.POST, "/api/v1/shows/**").hasAnyRole("ADMIN", "INTERNAL_SERVICE")
                .requestMatchers(HttpMethod.PUT, "/api/v1/shows/**").hasAnyRole("ADMIN", "INTERNAL_SERVICE")
                .requestMatchers(HttpMethod.DELETE, "/api/v1/shows/**").hasAnyRole("ADMIN", "INTERNAL_SERVICE")
                .requestMatchers(HttpMethod.PATCH, "/api/v1/shows/**").hasAnyRole("ADMIN", "INTERNAL_SERVICE")
